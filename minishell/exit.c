...
int		ft_exit()
{
	...
	{
		i = 1;
		while (i < g_data.command->n_tokens )
		{
			//ft_printf("--%s--\n",g_data.command->tokens[i]);
			if(!ft_just_numbers(g_data.command->tokens[i]))
			{
		...
		if(g_data.command->n_tokens == 2)
		{
			//ft_printf("%d\n", ft_atoi(g_data.command->tokens[1]));
			if(ft_atoi(g_data.command->tokens[1]) == -1 && ft_strncmp(g_data.command->tokens[1], "-1", 3))
				ft_printf("minishell: exit: %s: numeric argument required\n", g_data.command->tokens[1]);
			exit(ft_atoi(g_data.command->tokens[1]));
		}
		else
